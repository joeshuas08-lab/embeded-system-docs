...
static void bq25890_charger_external_power_changed(struct power_supply *psy)
{
	...
	case POWER_SUPPLY_USB_TYPE_DCP:
		input_current_limit = bq25890_find_idx(1000000, TBL_IINLIM);
...
static void bq25890_shutdown(struct i2c_client *client)
{
	...
	regmap_field_write(bq->rmap_fields[F_IINLIM], 0x1F); // 0x1F = 1.5A
	//regmap_field_write(bq->rmap_fields[F_VREG], 0x2D); // Limit Default//Limit 0x2D = 4.9V, offset default
	regmap_field_write(bq->rmap_fields[F_ICHG], 0x3F);  // 1
	if (client->irq >= 0)
